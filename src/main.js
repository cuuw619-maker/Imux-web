...
import {BLOCK_ORDER,textureUrl} from "./blocks.js";
...
function setInventory(open){
  ...
  help.classList.toggle("hidden",open||!menuOpen&&!controls.isLocked);
  if(open)controls.unlock();
...
function buildInventorySlots(){
  ...
  for(let i=0;i<36;i++){
    ...
    const column=i%9;
    button.style.left=`${8+column*18}px`;
    button.style.top=`${18+row*18}px`;
    const count=document.createElement("span");
    count.className="item-count";
    button.append(count);
    inventorySlots.append(button);
  }
}
...
function updateAttack(){
  ...
  }else if(attackIndicator&&!attackFull.hidden&&now<attackFlashEnd){
    attackProgress.style.clipPath="inset(100% 0 0 0)";
  }else if(now>=attackFlashEnd){
    attackIndicator.classList.add("hidden");
    attackFull.classList.add("hidden");
  }

  if(now>=attackEnd&&now<attackEnd+90){
...
function breakBlock(){
  ...
  if(!world.removeBlock(block.x,block.y,block.z))return;
}

function playerBox(x=camera.position.x,y=camera.position.y,z=camera.position.z){
  return {
    minX:x-PLAYER_RADIUS,maxX:x+PLAYER_RADIUS,
    minY:y-EYE_HEIGHT,maxY:y-EYE_HEIGHT+PLAYER_HEIGHT,
...
function moveHorizontal(dx,dz){
  ...
  camera.position.x=THREE.MathUtils.clamp(camera.position.x+dx,-WORLD_LIMIT,WORLD_LIMIT);
  if(world.collidesPlayer(camera.position.x,camera.position.y,camera.position.z,PLAYER_RADIUS,EYE_HEIGHT,PLAYER_HEIGHT))camera.position.x=oldX;

  const oldZ=camera.position.z;
  camera.position.z=THREE.MathUtils.clamp(camera.position.z+dz,-WORLD_LIMIT,WORLD_LIMIT);
  if(world.collidesPlayer(camera.position.x,camera.position.y,camera.position.z,PLAYER_RADIUS,EYE_HEIGHT,PLAYER_HEIGHT))camera.position.z=oldZ;
}

function moveVertical(dy){
  const oldY=camera.position.y;
  camera.position.y=oldY+dy;
  if(!world.collidesPlayer(camera.position.x,camera.position.y,camera.position.z,PLAYER_RADIUS,EYE_HEIGHT,PLAYER_HEIGHT))return false;

  let lo,hi;
  if(dy<0){
    lo=camera.position.y; hi=oldY;
    for(let i=0;i<14;i++){
      const mid=(lo+hi)*.5;
      camera.position.y=mid;
      if(world.collidesPlayer(camera.position.x,mid,camera.position.z,PLAYER_RADIUS,EYE_HEIGHT,PLAYER_HEIGHT))lo=mid;else hi=mid;
    }
    camera.position.y=hi;
    grounded=true;
  }else{
    lo=oldY; hi=camera.position.y;
    for(let i=0;i<14;i++){
      const mid=(lo+hi)*.5;
      camera.position.y=mid;
      if(world.collidesPlayer(camera.position.x,mid,camera.position.z,PLAYER_RADIUS,EYE_HEIGHT,PLAYER_HEIGHT))hi=mid;else lo=mid;
    }
...
function updateHunger(){
  hungerIcons.forEach((icon,i)=>{
    ...
    if(value>=2)image=hungerEffect?"food_full_hunger.png":"food_full.png";
    else if(value===1)image=hungerEffect?"food_half_hunger.png":"food_half.png";
    else if(value<=0)image=hungerEffect?"food_empty_hunger.png":"food_empty.png";
    icon.style.backgroundImage=`url("${textureRoot.toString()+"gui/"+image}")`;
  });
}

function buildMenuOverlay(){
  const overlay=document.querySelector("#menu-overlay");
  overlay.style.backgroundImage=`url("${textureRoot.toString()+"gui/title/background/lakeside_sunset_panorama_overlay.png"}")`;
}

function animateMenu(){
  const now=performance.now();
  panoramaCube.rotation.y=now*0.000025;
  panoramaCube.rotation.x=THREE.MathUtils.degToRad(2+Math.sin(now*.00012)*1.25);
